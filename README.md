...
이 저장소는 AI 에이전트(PM, Architect, Dev, QA)가 프로젝트 기획부터 검증까지 **순차적이고 체계적인 소프트웨어 개발 라이프사이클(SDLC)**을 수행하도록 유도하는 **에이전트 워크플로우 지침(Prompt Templates)** 모음입니다.
...
└── workflows/
    ...
    ├── 02_design_architect.md    # 2단계: 시스템 설계 및 아키텍쳐 (Architect)
...
*   **주요 산출물:** `docs/Architecture.json` (아키텍처 스키마)

### 🚀 3단계: 시스템 구현 및 개발 (`03_implementation_dev.md`)
*   **담당:** Developer (Software Engineer)
*   **역할:** 설계도(`Architecture.json`)를 엄격히 준수하여 실행 가능한 응용 코딩 및 모듈 결합도 관리
...
*   **역할:** 개발된 코드 구동검증, 최초 명세 대비 기능 동작 비교 및 오류 자가개선(Self-Refinement) 루프 가동
*   **주요 산출물:** `docs/Test_Report.md` (보고서 및 피드백 전파)
...
1. **프로젝트 시작 (1단계 - 기획):** 초기 💡요구사항을 상세히 적은 프롬프트와 함께 `01_planning_pm.md` 워크플로우를 호출합니다. (예: `/01_planning_pm 다음 요구사항으로 프로젝트를 기획해줘: [요구사항 상세]`)
2. **산출물 검토:** 에이전트가 요구사항을 분석하여 `docs/Spec.md` 및 `Blueprint` 산출물을 생성하면, 내용을 검토하고 확정합니다.
3. **순차적 단계 진행:** 기획 단계 산출물이 확정되면 다음 단계인 `02_design_architect.md`를 호출하여 아키텍처 설계를 진행합니다. 동일한 방식으로 산출물 확인 후 `03_implementation_dev.md` (구현), `04_verification_qa.md` (QA 및 검증) 순으로 단계별 턴(Turn)을 분리하여 순차적으로 진행하시면 됩니다.
